...
use Shoukhin\Forte\Exception\ForteConfigurationException;
use Shoukhin\Forte\Exception\ForteConnectionException;
...
class ForteHttpConnection
{
     ...
     * @param array            $config
     */
    public function __construct(ForteHttpConfig $httpConfig, array $config)
    {
        if (!function_exists("curl_init")) {
            throw new \Exception("Curl module is not available on this system");
        }
        $this->httpConfig = $httpConfig;
        $this->logger = ForteLoggingManager::getInstance(__CLASS__);
    }
     ...
     * @return mixed
     * @throws ForteConnectionException
     */
    public function execute($data)
    {
        ...
        if (curl_errno($ch)) {
            $ex = new ForteConnectionException(
        ...
        if ($httpStatus < 200 || $httpStatus >= 300) {
            $ex = new ForteConnectionException(
                $this->httpConfig->getUrl(),
                "Got Http response code $httpStatus when accessing {$this->httpConfig->getUrl()}. The response: {$result}",
                $httpStatus
            );
